...
public class Player extends Entity implements Runnable{
	...
	public Player(float x, float y, int width, int height, Playing playing) {
		...
		loadAnimations();
		initHitBox(x, y, (int)(23 * GameClass.SCALE), (int)(27 * GameClass.SCALE));
		initAttackBox();
	}
	...
	private void setAnimation() {
		...
		if (inAir) {	// set jumping animation
			if(airSpeed < 0)
				playerAction = JUMP;
			else
				playerAction = IDLE;
		...
		if(currentHealth <= 0)	// if player has died
			playerAction = DEAD;
	...
	private void loadAnimations() {
		...
		BufferedImage spriteAtlas = LoadnSave.getSpriteAtlas(LoadnSave.PLAYER_ATLAS_IMAGE);		// get player atlas 
		animations = new BufferedImage[7][8];  		//initialize animations array (8 rows/10 columns)
		for (int j = 0; j < animations.length; j++) {				// loop through rows
			for (int i = 0; i < animations[j].length; i++) {		// loop through columns
				...
				animations[j][i] = spriteAtlas.getSubimage(i * 64, j * 40, 64, 40);	
			}
		}
